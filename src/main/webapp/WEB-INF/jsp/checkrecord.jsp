...
<form id="form1" action="${pageContext.request.contextPath}/Wage/addreward" method="post">
    <tr>
        <td>薪酬发放单号:${requestScope.get('record2').getNo()}</td>
        <td><input type="submit" value="提交"></td><br>
        <td>机构：${requestScope.get('record2').getFirst()}/${requestScope.get('record2').getSecond()}
            /${requestScope.get('record2').getThird()}</td><br>
        <td>总人数：${requestScope.get('record2').getTotalPerson()}</td>
        <td>基本薪酬总额：${requestScope.get('record2').getTotal()}</td>
        <p>应付金额：</p><td><input type="text" value="0" name="m" id="m3"></td></td><br>
        <input type="hidden" name="n" value="${requestScope.get('record2').getNo()}">
        <input type="hidden" name="c" value="2">
    </tr>
    <c:forEach var="it" items="${requestScope.get('recordNo1')}" varStatus="s">
        <tr>
            ...
            <td>奖励奖金：<input type="text" value="${requestScope.get('奖励奖金').getAmount()}" name="amount" id="m1" oninput="calcuate()"></td>
            <input type="hidden" value="奖励奖金" name="wagename">
            <input type="hidden" value="${it.getName()}" name="Name">
            <input type="hidden" value="${requestScope.get('record2').getNo()}" name="No">
            <td>应扣奖金：<input type="text" value="${requestScope.get('应扣奖金').getAmount()}" name="amount" id="m2" oninput="calcuate()"></td>
...
<script>
    ...
    function calcuate(){
        ...
        c.value=Number(y)+Number(${requestScope.get('record2').getTotal()});
    }
</script>
</body>
</html>
